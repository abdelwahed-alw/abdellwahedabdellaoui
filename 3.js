  const themeToggle = document.getElementById('themeToggle');
       ...
       const key = element.dataset.en || element.dataset.fr || element.dataset.ar || element.textContent.trim();
        if (translations[key] && translations[key][lang]) {
          if (element.innerHTML.includes('<strong>') || element.innerHTML.includes('<b>')) {
             element.innerHTML = translations[key][lang]; // Use innerHTML to preserve HTML tags
          } else {
             element.textContent = translations[key][lang]; // Otherwise, use textContent
          }
        }
      });
      
      // Translate elements with data-key (like skill category titles)
    ...
    // --- START: EmailJS Contact Form Logic ---
    ...
    (function() {
        // The user ID you provided is used here as the Public Key
        emailjs.init({
          publicKey: "abdellwahed-alo",
        });
    })();
    ...
    contactForm.addEventListener('submit', function(e) {
        ...
        // Change button to show sending status
        ...
        // --- IMPORTANT: Replace with your actual Service and Template IDs from EmailJS ---
    ...
    // --- END: EmailJS Contact Form Logic ---
    ...
    function renderProjects(filter = 'all') {
      ...
      const filteredProjects = filter === 'all'
        ? projectsData
        : projectsData.filter(project => project.category === filter || (filter === 'other' && !['completed', 'in-progress'].includes(project.category)));
        ...
        projectCard.innerHTML = `
          ...
          <div class="project-content">
            ...
            <div class="project-links">
              <a href="${project.demo}" target="_blank" class="btn btn-outline" style="padding: 0.5rem 1rem; font-size: 0.9rem;">
                <i class="fas fa-external-link-alt"></i> <p data-en="Live Demo" data-fr="Démo en direct" data-ar="عرض حي">${translations["Live Demo"][currentLang]}</p>
              </a>
              <a href="${project.code}" target="_blank" class="btn btn-outline" style="padding: 0.5rem 1rem; font-size: 0.9rem;">
                <i class="fab fa-github"></i> <p data-en="Source Code" data-fr="Code source" data-ar="التعليمات البرمجية المصدرية">${translations["Source Code"][currentLang]}</p>
              </a>
            </div>
          </div>
        `;
    ...
    function loadFonts() {
      const fontUrls = [
        { name: 'Tajawal', url: 'https://fonts.gstatic.com/s/tajawal/v9/Iurf6YBj_oCad4k1l_6gLrZjiY.woff2' },
        { name: 'Courier Prime', url: 'https://fonts.gstatic.com/s/courierprime/v1/HI_KiYtLzOqQo7jCg5cxHvKqg_M.woff2' },
        { name: 'Poppins', url: 'https://fonts.gstatic.com/s/poppins/v20/pxiByp8kv8PHlEeNjjjFcZfS.woff2' }
      ];
    ...
    class PortfolioAnalytics {
      constructor() {
        this.visitCount = localStorage.getItem('portfolioVisitCount') || 0;
        this.lastVisit = localStorage.getItem('portfolioLastVisit') || null;
        this.init();
      }
      ...
      incrementVisitCount() {
        this.visitCount = parseInt(this.visitCount) + 1;
        localStorage.setItem('portfolioVisitCount', this.visitCount);
      }

      recordLastVisit() {
        const now = new Date();
        this.lastVisit = now.toISOString();
        localStorage.setItem('portfolioLastVisit', this.lastVisit);
      }

      displayVisitInfo() {
        const visitInfo = document.createElement('div');
        visitInfo.className = 'visit-info';
        visitInfo.style.position = 'fixed';
        visitInfo.style.bottom = '10px';
        visitInfo.style.right = '10px';
        visitInfo.style.backgroundColor = 'var(--dark-light)';
        visitInfo.style.padding = '0.5rem 1rem';
        visitInfo.style.borderRadius = '0.5rem';
        visitInfo.style.fontSize = '0.8rem';
        visitInfo.style.color = 'var(--light)';
        visitInfo.style.opacity = '0.7';
        visitInfo.style.zIndex = '1000';
        
        const lastVisitDate = this.lastVisit ? new Date(this.lastVisit) : new Date();
        const options = { year: 'numeric', month: 'short', day: 'numeric' };
        const formattedDate = lastVisitDate.toLocaleDateString('en-US', options);
        
        visitInfo.textContent = `Visits: ${this.visitCount} | Last: ${formattedDate}`;
        document.body.appendChild(visitInfo);
      }
    }
